...
@section('title', 'Yeni Destinasyon')

@section('content')
<div class="row mb-8">
    <div class="col-md-12">
        <div>
            <h2>Yeni Destinasyon Oluştur</h2>
            <nav aria-label="breadcrumb">
                <ol class="breadcrumb mb-0">
                    ...
                    <li class="breadcrumb-item active" aria-current="page">Yeni Destinasyon</li>
                </ol>
            </nav>
        </div>
    </div>
...
<form action="{{ route('destinations.store') }}" method="POST" enctype="multipart/form-data" class="row g-6">
    @csrf
            ...
            <div class="card-body p-6 d-flex flex-column gap-4">
                ...
                <div>
                    ...
                    <input type="text" 
                           ...
                           name="name" 
                           value="{{ old('name') }}"
                           placeholder="Destinasyon adı (örn: Paris, İstanbul)" 
                           required>
                    @error('name')
                ...
                <div>
                    ...
                    <input type="text" 
                           ...
                           name="slug" 
                           value="{{ old('slug') }}"
                           placeholder="destinasyon-adi (otomatik oluşturulur)">
                    <small class="form-text text-muted">Boş bırakılırsa destinasyon adından otomatik oluşturulur</small>
                    @error('slug')
                        <div class="invalid-feedback">{{ $message }}</div>
                    @enderror
                </div>
                ...
                <div>
                    ...
                    <textarea class="form-control @error('description') is-invalid @enderror" 
                              ...
                              name="description" 
                              rows="5"
                              placeholder="Destinasyon hakkında kısa açıklama">{{ old('description') }}</textarea>
                    @error('description')
                        <div class="invalid-feedback">{{ $message }}</div>
                    @enderror
                </div>
...
document.addEventListener('DOMContentLoaded', function() {
    // Destinasyon adı değiştiğinde slug'ı otomatik oluştur
